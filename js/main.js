...
function suc(a) {
  ...
  //  document.getElementById('accel_x').innerHTML = "a.x = " + a.x + ", a.y = " + a.y;
    ...
    try { 
        ...
        //debug Probably undefined
    }
      
      
    //if (intel.xdk.iswin8 === undefined || intel.xdk.iswp8 === undefined){
    if(!win8) {
        if(container!=null){ 
            container.style.webkitTransform = 'translate(' + current_left + 'px, ' + current_top + 'px) rotate(' + canangle + 'deg) scale(.85, .85)';
        }
        if(label!=null) {
            label.style.webkitTransform = 'translate(' + labelleft + 'px, 0px)';
        }
    }
    else {
        //if (intel.xdk.iswin8 || intel.xdk.iswp8)
        if(win8){
            if(container!=null) container.style.transform = 'translate(' + current_left + 'px, ' + current_top + 'px) rotate(' + canangle + 'deg) scale(.85, .85)';
        }
        else {
        ...
        //if (intel.xdk.iswin8 || intel.xdk.iswp8)
        if(win8){
            if(label!=null) label.style.transform = 'translate(' + labelleft + 'px, 0px)';
        }
        else{
